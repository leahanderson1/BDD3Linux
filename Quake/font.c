...
#define UTF8_NUM_BUFFS 4        // Number of rotating buffers
#define UTF8_BUFFERLEN 1024     // Size of each buffer (should be large enough for your strings)

/**
 * Returns a pointer to the next available buffer in a rotating set.
 * Each call advances the index, reusing buffers in a circular manner.
 */
static char* get_utf8_buffer(void) {
	static char utf8_buffers[UTF8_NUM_BUFFS][UTF8_BUFFERLEN];
	static int buffer_idx = 0;
	buffer_idx = (buffer_idx + 1) & (UTF8_NUM_BUFFS - 1); // Wrap around using bitwise AND
	return utf8_buffers[buffer_idx];
}

uint32_t utf8_decode_nth(const char* input,
	int        index,
	size_t     length)
{
	const unsigned char* p = (const unsigned char*)input;
	const unsigned char* limit = p + length;   /* one-past-end sentinel   */
	uint32_t code = ' ';                        /* default result          */

	while (p < limit && *p)                    /* stop at NUL or limit    */
	{
			...
			(p[3] & 0xC0) == 0x80)
		{
			...
			code = '?';
			p += 1;  /* skip one byte and resynchronise */
		}

		/* -------------------- reached desired code-point? ------------- */
		if (index-- == 0)
			return code;
	}

	/* Ran out of text before reaching the index -> return space. */
	return ' ';
}

/**
 * Converts a string from the system's default code page (ANSI) to UTF-8.
...
int copy_glyph(UnicodeBlock* block, SDL_Surface* glyph, SDL_Surface* destination, int columns, uint32_t codepoint)
{
	if (!glyph)
	{
		return 1; // Skip if rendering fails (e.g., no glyph)
	}
	// Calculate position in grid
...
int generate_font_pngs(void)
{
	...
	for (int i = 0; i < num_blocks; i++)
	{
		...
		uint32_t num_chars = block->end - block->start + 1;
		const int rows = (int)ceil((double)num_chars / COLUMNS);

		// glyphs path (no extension)
		char* relative_filename = va("fonts/glyphs%i_%i", font_index, i);

		// Create atlas surface (no attempt to load here)
		...
		for (uint32_t codepoint = block->start; codepoint <= block->end; codepoint++)
		{
			if ((i == 0 || i == 1) && codepoint - block->start < 33)
			{
				// Copy from console bitmap for first block chars
				if (!consoleSurface)
				{
...
void draw_character_quad_ex_inner(int x, int y, uint32_t codepoint, int r, int g, int b)
{
