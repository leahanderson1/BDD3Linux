...
void Sbar_DrawStringRGBA(int x, int y, const char* str, const char* hex, float alpha)
{
	Draw_StringRGBA(x, y + 24, str, CL_PLColours_Parse(hex), alpha);
}
void Sbar_DrawString_Right(int x, int y, const char* str)
{
	Draw_String_Right(x, y + 24, str);
}

void Sbar_DrawStringRGBA_Right(int x, int y, const char* str, const char* hex, float alpha)
{
	Draw_StringRGBA_Right(x, y + 24, str, CL_PLColours_Parse(hex), alpha);
}
/*
===============
...
void Sbar_DrawInventorySA(void)
{
...
===============
*/
static qpic_t* Sbar_FacePicEx(int x, int y)
{
	int f, anim;
	qpic_t* face_pic;
	...
	Sbar_DrawPic(x, y, face_pic);

	// Handle damage hue first (highest priority)
	if (cl.time <= cl.faceanimtime && cl_damagehue.value && cl_damagehuecolor.value)
	{
		plcolour_t color = CL_PLColours_Parse(cl_damagehuecolor.string);
		Draw_FillPlayer(x, y + 24, 24, 25, color, 0.2);
	}
	else
	{
		Draw_PowerupOverlays(x, y);
	}

	return face_pic;
}

static qpic_t* Sbar_FacePic()
{
	int f, anim;
	qpic_t* face_pic;
...
void Sbar_SoloScoreboardSA(void)
{
	...
	if (cl.gametype != GAME_DEATHMATCH)
	{
		sprintf(str, "%d\\%d", cl.stats[STAT_MONSTERS], cl.stats[STAT_TOTALMONSTERS]);
		if (strlen(str) >= 6) {
			glPushMatrix();
			const float scale = 0.75f;
			const float invScale = 1.0f / scale;
			glScalef(scale, scale, scale);
			Sbar_DrawStringRGBA_Right(276 * invScale, 20 * invScale, str, "0x4F9B42", 1.0f);
			glPopMatrix();
		}
		else {
			Sbar_DrawStringRGBA_Right(276, 14, str, "0x4F9B42", 1.0f);
		}
	}
}

void Sbar_DrawItemsSA(int x, int y) {
	for (int i = 0; i < 2; i++)
	{
		if (cl.items & (IT_KEY1 << i))
		{
